...
public class ServicebusUtil implements IMessageRelay {
	...
	private String urlSuffix  = ".servicebus.windows.net";
	...
	public ServicebusUtil() {
		...
		try {
			namespace  = ConfigManager.envVar(AZURE_SERVICEBUS_NAMESPACE);
			keyName    = ConfigManager.envVar(AZURE_SERVICEBUS_KEY_NAME);
			accessKey  = ConfigManager.envVar(AZURE_SERVICEBUS_ACCESS_KEY);
			queueName  = ConfigManager.envVar(AZURE_SERVICEBUS_QUEUE, DEFAULT_OUTPUT_QUEUE);
			urlSuffix  = ".servicebus.windows.net";
    ...
    public boolean putMessageOnQueue(String messageText) {

    	try {
    		logger.debug("putMessageOnQueue: " + messageText);
